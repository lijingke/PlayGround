...
class QuestionnaireHeadView: UIView {
    ...
    override func draw(_ rect: CGRect) {
        super.draw(rect)
        sectionHead.addCorner(conrners: [.topLeft, .topRight], radius: 20)
    }
    ...
    lazy var sectionTitle: UILabel = {
        let label = UILabel()
        label.textColor = UIColor(hex: 0x152644)
        label.font = UIFont.semibold(20)
        label.text = "梅毒知识科普"
        return label
    }()
    
    lazy var sectionTitleLine: UIView = {
        let view = UIView()
        view.backgroundColor = UIColor(hex: 0xE7F0FF)
        return view
    }()
}

// MARK: - Setup
extension QuestionnaireHeadView {
    ...
    private func setupUI() {
        ...
        bgImageView.addSubview(labelLine)
        bgImageView.addSubview(contentLable)
        bgImageView.addSubview(summarizerLabel)
        ...
        addSubview(sectionHead)
        sectionHead.snp.makeConstraints { (make) in
            make.top.equalTo(contentLable.snp.bottom).offset(21.5)
            make.left.bottom.right.equalToSuperview()
        }
        sectionHead.addSubview(sectionTitleLine)
        sectionHead.addSubview(sectionTitle)
        sectionTitle.snp.makeConstraints { (make) in
            make.top.equalToSuperview().offset(23)
            make.left.equalToSuperview().offset(21)
            make.bottom.equalToSuperview().offset(-10)
        }
        sectionTitleLine.snp.makeConstraints { (make) in
            make.left.equalTo(sectionTitle.snp.left).offset(-6)
            make.right.equalTo(sectionTitle.snp.right).offset(6)
            make.bottom.equalTo(sectionTitle)
            make.height.equalTo(10)
        }
        bringSubviewToFront(sectionTitle)
